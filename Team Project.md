...
- PPT를 어떻게 구성할 것인가?
- 아이디 수정에서 주소는 어떻게 되고 있는가?
- api가 활동하고 지연시간동안 어떻게 할 것인가?(진행되고 있다는 표시를 나중에 추가할 예정임)
- 지도에 뿌려졌을 때 확대 축소가 가능한가?(가능함)
  - 지도에 뿌려진 내용들이 더 잘 보이게 하기
  - 지도에 있는 내용 리스트들 고치기.
  - 사이트의 사용자가 누구이며 ?
  - 가입을 할 때 병원 관계자의 폼이 따로 있어야 함.
- 추가 내용을 정리해서 발전 방향을 제시할 것.
- 현재 기입한 기대효과는 1차효과임. 
- 유저들의 바람이나 욕구 등을 정리해서 작동시킬 것
- STT(방향성 제시)나 TTS등에서 설명이 존재해야 함.
  - 키보드 사용이 불가한 사람에게 
- 현재 접속한 IP를 통해서 주소를 불러올 수 있도록.
- 아이디어를 더 강조할 것. 사이트의 가치.
